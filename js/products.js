...
let categoryAll =localStorage.getItem(CATEGORY) || "all"

let search = "";

let activePage = 1;

function getProductCard({ images, name, description, id, price, rating,category }) {
  ...
  <div class="">
  <div class="cards">
     <a onclick="getProductPage('${id}')" href="../product.html"><img src="${images[0]}" class="card-img-top" alt="..."></a>
...
function getProducts() {
  ...
  if(categoryAll !== "all"){
    ...
    results = results.filter((el) => el.category === categoryAll)
  }
...
categories.forEach(({ name }) => {
  categorySelect.innerHTML += `<option ${categoryAll === name ? 'selected' : ''} value=${name}>${name}</option>`;
});
